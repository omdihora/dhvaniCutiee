...
import { Heart, Sparkles, Code2, Play, Ribbon } from 'lucide-react';
...
export const HeroWelcome: React.FC<HeroWelcomeProps> = ({ onStartQuiz }) => {
  ...
  return (
    <motion.div
      ...
      className="min-h-[85vh] w-full flex items-center justify-center p-4 sm:p-6 relative z-10"
    >
      {/* Floating Interactive Star Easter Eggs near Hero */}
      <StarEasterEgg message="I miss you ❤️" top="18%" left="14%" />
      <StarEasterEgg message="You're my favorite notification ❤️" top="24%" right="15%" />
      <StarEasterEgg message="Made with love ❤️" bottom="20%" left="16%" />
      <StarEasterEgg message="My favorite bug is falling for you ❤️" bottom="24%" right="17%" />

      {/* Main Apple Glass Card */}
      <div className="w-full max-w-2xl glass-card-apple rounded-[36px] p-8 sm:p-12 text-center border border-white/20 shadow-[0_20px_80px_rgba(236,72,153,0.25)] relative overflow-hidden group">
        ...
        >
          ...
          <span>DEVELOPER_EDITION // DHVANI_OS</span>
        </motion.div>
        ...
        >
          This website wasn't written with just <span className="font-code text-pink-200 font-normal">HTML</span>,{' '}
          <span className="font-code text-purple-200 font-normal">CSS</span>, and{' '}
          <span className="font-code text-amber-200 font-normal">JavaScript</span>...
          <br className="hidden sm:block" />
          It was written with <span className="text-pink-200 font-medium underline decoration-pink-300/60 underline-offset-4">countless thoughts about you.</span>
        </motion.p>

        {/* Glowing Pill CTA Button: Initialize Love Protocol */}
        <motion.div
        ...
        >
          ...
          >
            ...
            <span className="relative px-9 py-4 rounded-full bg-[#160a24] text-white flex items-center gap-3 transition-all duration-300 group-hover/btn:bg-transparent group-hover/btn:text-white">
              ...
              <span className="tracking-wide font-medium">Initialize Love Protocol</span>
              <Play className="w-4 h-4 text-pink-300 fill-pink-300 group-hover/btn:translate-x-1 transition-transform" />
            </span>
          </button>
        </motion.div>
